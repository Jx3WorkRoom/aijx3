...
//计算天数差的函数
function diffDate(sDate1, sDate2){ //sDate1和sDate2是2002-12-18格式
    if ($.isEmpty(sDate1)) {
        return;
    }
    if ($.isEmpty(sDate2)) {
    ...
    oDate1=$.parseDate(sDate1);
    oDate2=$.parseDate(sDate2);
    //aDate = sDate1.split("-");
...
function getYearOfMonth(start, end, day) {
    //var start = '2010-09-20';
    //var end = '2011-12-04';
    //var day = 365;
    if (day < 15) return '';
    if (day > 15 && day < 31) return '1月';
    var start_date = Date.parseDate(start, 'Y-m-d');
    var end_date = Date.parseDate(end, 'Y-m-d');
    var months = getMonthCZ(start_date, end_date);
    var temp = start_date.add(Date.MONTH, months);
    var days = diffDate(temp.format('Y-m-d'), end).days;
    if (days >= 15) {
        months += 1;
    ...
    if (months < 12) {
        ...
        var years = Math.floor(months / 12) + '年';
        if (months % 12) {
            years += (months % 12) + '月';
        }
        return years;
