...
function initTable(url,keyNum) {
    ...
    if(url==null) {
        ...
        if (shape == "" && areaSelection == "") {
            url = api + 'levelingList?needType=' + encodeURI(tradeType) + '&startNum=' + encodeURI(startNum) + '&endNum=' + encodeURI(endNum);
        }else{
            url = api + 'levelingList?needType=' + encodeURI(tradeType)
...
//加载搜索框
function initSeach() {
    var url = api+'levelingListSelection';
    $.getJSON(url,function (data) {
